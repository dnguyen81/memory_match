...
function card_create(){
    var i = 0;
    console.log('card creation clicked');
    while (i<front){
        var random_index=Math.floor(Math.random()*(front.length));
        create_card_container(front[random_index]);
        i++;
        front.splice(random_index,1);
        return random_index;
...
console.log('create card container clicked');
   for (var i =0; i<front.length; i++){
        ...
        front.splice(i,1);
       var random_card_array = front.concat(new_card_array);
    ...
    var img_back = $('<img>').addClass('back').attr('src', 'images/rebel.png', 'onclick', 'card_clicked(this)');
